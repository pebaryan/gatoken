"""
Scale Experiment: Larger training data + bigger vocabulary
"""

from gatoken import RotorSubwordTokenizer, compute_metrics

# Extended training data (synthetic but more diverse)
EN = [
    ...
    # Additional sentences for scale
    "The sun rises in the east and sets in the west.",
    "She enjoys reading books about history and science.",
    "They decided to travel to the mountains during the holiday.",
    "The new restaurant serves delicious local cuisine.",
    "He is learning to play the guitar in his free time.",
    "The museum has an interesting exhibition about ancient civilizations.",
    "We need to finish this project before the end of the month.",
    "She bought a beautiful dress for the wedding party.",
    "The train arrived late because of the heavy rain.",
    "He likes to go jogging every morning before work.",
]
...
ID = [
    ...
    "Dia senang membaca buku tentang sejarah dan sains.",
    "Mereka memutuskan untuk pergi ke gunung saat liburan.",
    "Restoran baru itu menyajikan masakan lokal yang lezat.",
    "Dia sedang belajar bermain gitar di waktu luangnya.",
    "Museum itu memiliki pameran menarik tentang peradaban kuno.",
    "Kita perlu menyelesaikan proyek ini sebelum akhir bulan.",
    "Dia membeli gaun cantik untuk pesta pernikahan.",
    "Kereta api datang terlambat karena hujan deras.",
    "Dia suka berlari pagi setiap hari sebelum bekerja.",
]
...
def main():
    print("Training with larger dataset...")
    train = EN + ID + ZH
    tok = RotorSubwordTokenizer(max_vocab_size=16000)
    tok.train(train)

    print(f"Vocab size: {tok.vocab_size}")
    print(f"Merges learned: {len(tok.merges)}\n")

    en_m = compute_metrics(tok, EN, "en")
    id_m = compute_metrics(tok, ID, "id")
    zh_m = compute_metrics(tok, ZH, "zh")

    print("=== Scaled Results ===")
    print(f"English    : fertility={en_m.fertility:.3f}")
    print(f"Indonesian : fertility={id_m.fertility:.3f}")
    print(f"Chinese    : fertility={zh_m.fertility:.3f}")

    print("\nRatios:")
    print(f"ID/EN  : {id_m.fertility / en_m.fertility:.3f}")
    print(f"ZH/EN  : {zh_m.fertility / en_m.fertility:.3f}")
    print(f"ZH/ID  : {zh_m.fertility / id_m.fertility:.3f}")
